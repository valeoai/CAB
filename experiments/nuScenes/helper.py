...
def load_model_yaml(model_dir, env, ts=100, device="cuda:0"):
    model_registrar = ModelRegistrar(model_dir, device)
    model_registrar.load_models(ts)
    path_to_yaml = Path(model_dir).joinpath('config.yaml')
    opts = Options(source=str(path_to_yaml), run_parser=False).options
    hyperparams = deep_asdict(Options(source=path_to_yaml, run_parser=False).options)
    ...
    for node in node_list:
        ...
        if node.type.name == 'VEHICLE':
            ...
            if node.id == "ego":
                ...
                car = cars[i % len(cars)]
            r_img = rotate(car, node.get(np.array([ts_key]), {'heading': ['°']})[0, 0] * 180 / np.pi,
                           reshape=True)
...
def plot_vehicle_history(ax,nodes,dt,max_hl,x_min,y_min):
    ts_key = list(nodes.keys())[0]
    histories_dict = {}
    for node in nodes[ts_key]:
        history = node.get(np.array([ts_key - max_hl, ts_key]), 
                           {'position': ['x', 'y']})  # History includes current pos
        history = history[~np.isnan(history.sum(axis=1))]

        histories_dict[node] = history
    cmap = ['k', 'b', 'y', 'g', 'r']
    line_alpha = 0.7
    line_width = 0.2
    edge_width = 2
