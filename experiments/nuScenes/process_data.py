...
def process_scene(ns_scene, env, nusc, data_path):
    ...
    scene = Scene(timesteps=max_timesteps + 1, dt=dt, 
                  name=str(scene_id), sample_tokens=sample_tokens, 
                  aug_func=augment, x_min=x_min, y_min=y_min)
    ...
    for node_id in pd.unique(data['node_id']):
        ...
        if node_df.iloc[0]['type'] == env.NodeType.VEHICLE and not node_id == 'ego': # Hedi: filter with kalman all the other cars' trajectories... don't know why
            # Kalman filter Agent
            vx = derivative_of(x, scene.dt)
            vy = derivative_of(y, scene.dt)
            ...
            global curv_0_1
            total += 1
            if pl > 1.0: # Hedi: This implementation seems strange: the value assigned for a node depends on the order of the nodes
                if curvature > .2:
                    curv_0_2 += 1
        ...
        if node_df.iloc[0]['type'] == env.NodeType.VEHICLE:
            ...
            data_dict = {('position', 'x'): x,
                         ...
                         ('acceleration', 'y'): ay,
                         ('acceleration', 'norm'): np.linalg.norm(np.stack((ax, ay), axis=-1), axis=-1),
                         ('heading', 'x'): heading_x, # Hedi: has to do with the speed vector, but i didnt understand exactly what it is
                         ('heading', 'y'): heading_y, # Hedi: has to do with the speed vector, but i didnt understand exactly what it is
                         ('heading', '°'): heading,
                         ('heading', 'd°'): derivative_of(heading, dt, radian=True)}
            node_data = pd.DataFrame(data_dict, columns=data_columns_vehicle)
        else:
