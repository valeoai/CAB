...
class MultimodalGenerativeCVAEReweight(object):
    def __init__(self,
                 env,
                 node_type,
                 model_registrar,
    ...
    def encoder(self, mode, x, y_e, branch, num_samples=None):
        ...
        z_entropy_p = latent.p_dist.entropy()
        z_entropy_q = latent.q_dist.entropy()
        ...
        return z, kl_obj, {"p": z_entropy_p, "q": z_entropy_q}
    ...
    def train_loss(self,
        ...
        # Cut gradient backpropagation
        if self.hyperparams["reweight_params"].get("detach_start", True):
            x_so = x_so.detach()
        else:
            x_so = x_so


        z, kl, z_entropies = self.encoder(mode, x, y_e, "full") # HEDI: kl between q(z|x,y) and p(z|x)
        z_so, kl_so, z_so_entropies = self.encoder(mode, x_so, y_e, "state")
        ...
        loss_w = self.hyperparams['reweight_params']['loss_weights']
        ...
        # Decrease the likelihood for easy examples for the state branch
        if self.hyperparams["reweight_params"].get("detach_end", False):
            log_p_y_xz_so_mean = log_p_y_xz_so_mean.detach()

        gamma = self.hyperparams["reweight_params"].get("gamma", 1.)
        weight = torch.pow(1./(1. + torch.exp(log_p_y_xz_so_mean)), gamma)
        loss += - loss_w["product"] * torch.mean( weight * log_p_y_xz_full_mean )

        # Maximize the entropy of z to encourage diversity
        if self.hyperparams["reweight_params"].get("z_entropy", False):
            z_entropy = 0.
            if self.hyperparams["reweight_params"]["z_entropy_p"]:
                z_entropy += torch.mean(z_entropies["p"])
            if self.hyperparams["reweight_params"]["z_entropy_q"]:
                z_entropy += torch.mean(z_entropies["q"])
            loss += - self.hyperparams["reweight_params"]["z_entropy_weight"] * z_entropy
    ...
    def eval_loss(self,
        ...
        z, _, _ = self.encoder(mode, x, y_e, branch="full")  # [k_eval, nbs, N*K]
    ...
    def predict(self,
        ...
        z, num_samples, num_components = self.latent_full.sample_p(num_samples,
                                                              mode,
        ...
        mode_prob = self.latent_full.p_dist.probs
