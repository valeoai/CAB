...
class MultimodalGenerativeCVAECAB(object):
    def __init__(self,
                 env,
                 node_type,
                 model_registrar,
    ...
    def obtain_encoded_tensors(self,
        ...
        fixed_input_tensors = self.hyperparams['cab_params'].get('fixed_input_tensors',False)
        ...
        if self.hyperparams['use_map_encoding'] and self.node_type in self.hyperparams['map_encoder']:
            ...
            if fixed_input_tensors:
                map_so = torch.zeros_like(map)
                map_so[:,0] = 1
                encoded_map_so = self.node_modules[self.node_type + '/map_encoder'](map_so*2 - 1., (mode == ModeKeys.TRAIN))    
            else:
                encoded_map_so = self.node_modules[self.node_type + '/map_encoder'](torch.zeros_like(map), (mode == ModeKeys.TRAIN))
        ...
        if self.hyperparams['edge_encoding']:
            ...
            x_concat_list_so.append(torch.zeros_like(total_edge_influence))  # [bs/nbs, 4*enc_rnn_dim]
        ...
        x_concat_list_so.append(node_history_encoded)  # [bs/nbs, enc_rnn_dim_history]
        ...
        if self.hyperparams['use_map_encoding'] and self.node_type in self.hyperparams['map_encoder']:
            ...
            x_concat_list_so.append(encoded_map_so)

        x = torch.cat(x_concat_list, dim=1)
        x_so = torch.cat(x_concat_list_so, dim=1)
    ...
    def decoder(self, mode, x, x_so, x_nr_t, y, y_r, n_s_t0, z, z_so, labels, prediction_horizon, num_samples):
        ...
        return log_p_y_xz_so, log_p_y_xz
    ...
    def train_loss(self,
        ...
        use_q = self.hyperparams['cab_params'].get('use_q', False)
        if not use_q:
            detached_latent_state = td.OneHotCategorical(logits=self.latent_state.p_dist.logits.detach())
            kl_full_state = td.kl.kl_divergence(self.latent_full.p_dist, 
                                                detached_latent_state) # I want to encourage latent_full to englobe latent_state
            entropy_full = self.latent_full.p_dist.entropy().mean()

        else:
            detached_latent_state = td.OneHotCategorical(logits=self.latent_state.q_dist.logits.detach())
            kl_full_state = td.kl.kl_divergence(self.latent_full.q_dist, 
                                                detached_latent_state) # I want to encourage latent_full to englobe latent_state
            entropy_full = self.latent_full.q_dist.entropy().mean()
        kl_full_state = torch.mean(kl_full_state)

        log_p_y_xz_so, log_p_y_xz_full = self.decoder(mode, x, x_so, x_nr_t, y, y_r, n_s_t0, z, z_so,
        ...
        kl_fs_weight = self.hyperparams['cab_params']['kl_fs_weight']
        so_weight = self.hyperparams['cab_params'].get('so_weight', 1.)
        loss_full = -log_likelihood_full + self.kl_weight*kl - mutual_inf_p_full
        loss_so = -log_likelihood_so + self.kl_weight*kl_so - mutual_inf_p_so      
        loss = so_weight*loss_so + loss_full - kl_fs_weight*kl_full_state

        entropy_weight = self.hyperparams['cab_params'].get('entropy_weight',0.)
        if entropy_weight > 0.:
            loss = loss - entropy_weight * entropy_full
        ...
        if self.log_writer is not None:
            ...
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'mutual_information_p_so'),
                                       mutual_inf_p_so,
                                       self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'kl_full_state'),
                                       kl_full_state,
                                       self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'entropy_full'),
                                       entropy_full,
                                       self.curr_iter)
            ...
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'll_so'),
                                       log_likelihood_so,
                                       self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'loss_full'),
                                       loss_full,
                                       self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'loss_so'),
                                       loss_so,
                                       self.curr_iter)
    ...
    def get_shapley_values(self,
        ...
        edge_size = self.eie_output_dims
        img_size = self.hyperparams['map_encoder'][self.node_type]['output_size']
        state_size = self.hyperparams['enc_rnn_dim_history']

        # order: edge, state, map

        node_hist_baseline = torch.zeros(x.size(0), 1, self.state_length, device=self.device)
        x_state_baseline = self.encode_node_history(mode, node_hist_baseline, torch.zeros(x.size(0), device=self.device).long())

        x_mask[:,edge_size:edge_size+state_size] = 1
        x_mask[:,edge_size+state_size:] = 2
        x_baseline[:,:edge_size] = x_so[:,:edge_size]
        x_baseline[:,edge_size:edge_size+state_size] = x_state_baseline # run the lstm on a list of zeros
        x_baseline[:,edge_size+state_size:] = x_so[:,edge_size+state_size:]

        o = self.shapley_forward(x_baseline, x_nr_t, y_r, n_s_t0, prediction_horizon, shapley_output)

        sv = ShapleyValues(self.shapley_forward)
        shapley_values = sv.attribute(x, 
                                     feature_mask=x_mask, 
                                     baselines=x_baseline, 
                                     additional_forward_args=(x_nr_t, y_r, n_s_t0, prediction_horizon, shapley_output))
        edge_shapley = shapley_values[:,0] # edge 
        state_shapley = shapley_values[:,edge_size] # state
        map_shapley = shapley_values[:,edge_size+state_size] # map
        return torch.stack((edge_shapley, state_shapley, map_shapley), dim=1)


    def shapley_forward(self, x_shapley, x_nr_t, y_r, n_s_t0, prediction_horizon, shapley_output):
        mode = ModeKeys.PREDICT
        self.latent_full.p_dist = self.p_z_x(mode, x_shapley, branch="full")
        z, num_samples, num_components = self.latent_full.sample_p(1,
                                                            mode,
                                                            most_likely_z=True,
                                                            full_dist=False,
                                                            all_z_sep=False)
        _, our_sampled_future = self.p_y_xz(mode, x_shapley, x_nr_t, y_r, n_s_t0, z,
                                            prediction_horizon,
                                            num_samples,
                                            "full",
                                            num_components,
                                            True)
        if shapley_output=="L2":
            out = torch.sqrt(torch.sum((our_sampled_future[0,:,-1,:] - our_sampled_future[0,:,0,:])**2, -1))
        return out
