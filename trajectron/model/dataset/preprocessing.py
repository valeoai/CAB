...
def get_node_timestep_data(env, scene, t, node, state, pred_state,
    ...
    if hyperparams['edge_encoding']:
        ...
        neighbors_data_st = dict() #HEDI: history of states of each connected node
        neighbors_edge_value = dict() #HEDI: edge value of each connnected node
...
def get_timesteps_data(env, scene, t, node_type, state, pred_state,
                       edge_types, min_ht, max_ht, min_ft, max_ft, hyperparams, ego_only=False, query_node=None):
    """
    ...
    if ego_only:
        _nodes_per_ts = {}
        for ts in nodes_per_ts:
            for node in nodes_per_ts[ts]:
                if node.id == "ego":
                    _nodes_per_ts[ts] = [node]
        nodes_per_ts = _nodes_per_ts
