...
class MultimodalGenerativeCVAE(object):
    ...
    def get_shapley_values(self,
        ...
        x_baseline[:,:edge_size] = 0.
        x_baseline[:,edge_size:edge_size+state_size] = x_state_baseline # run the lstm on a list of zeros
        x_baseline[:,edge_size+state_size:] = encoded_map_so # cnn on all-driveable image
    ...
    def shapley_forward(self, x_shapley, x_nr_t, y_r, n_s_t0, prediction_horizon, shapley_output):
        mode = ModeKeys.PREDICT
        self.latent.p_dist = self.p_z_x(mode, x_shapley)
        z, num_samples, num_components = self.latent.sample_p(1,
                                                            mode,
                                                            most_likely_z=True,
                                                            full_dist=False,
                                                            all_z_sep=False)
