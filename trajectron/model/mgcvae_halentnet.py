...
class MultimodalGenerativeCVAEHalentNet(object):
    def __init__(self,
                 env,
                 node_type,
                 model_registrar,
    ...
    def create_node_models(self):
        ...
        #   DCGAN Descriminator   #
        ###########################
        if self.node_type.name == 'VEHICLE':
            self.add_submodule(self.node_type + '/discriminator',
                               model_if_absent=DCGANDiscrim(
                                   self.hyperparams['map_encoder'][self.node_type]['map_channels'] + self.hyperparams['prediction_horizon'], 64, [100,100]))
    ...
    def obtain_encoded_tensors(self,
        ...
        if self.hyperparams['use_map_encoding'] and self.node_type in self.hyperparams['map_encoder']:
            ...
            encoded_map = self.node_modules[self.node_type + '/map_encoder'](map * 2. - 1., (mode in [ModeKeys.TRAIN, ModeKeys.GAN]))
            do = self.hyperparams['map_encoder'][self.node_type]['dropout']
            encoded_map = F.dropout(encoded_map, do, training=(mode in [ModeKeys.TRAIN, ModeKeys.GAN]))
        ...
        if mode in [ModeKeys.TRAIN, ModeKeys.EVAL, ModeKeys.GAN]:
    ...
    def encode_node_history(self, mode, node_hist, first_history_indices):
        ...
        outputs = F.dropout(outputs,
                            ...
                            training=(mode in [ModeKeys.TRAIN, ModeKeys.GAN]))  # [bs, max_time, enc_rnn_dim]
        ...
        if self.hyperparams['edge_influence_combine_method'] == 'sum':
            ...
            if len(encoded_edges) == 0:
                ...
                combined_edges = F.dropout(combined_edges,
                                           p=1. - self.hyperparams['rnn_kwargs']['dropout_keep_prob'],
                                           training=(mode in [ModeKeys.TRAIN, ModeKeys.GAN]))
    ...
    def p_y_xz(self, mode, x, x_nr_t, y_r, n_s_t0, z_stacked, prediction_horizon,
               num_samples, num_components=1, gmm_mode=False, z_2nd=None, switch_t=None):
        r"""
        ...
        :param z_2nd: If True: the z is switched during generation
        :param switch_t: The timestep to switch z
        ...
        if z_2nd is not None:
            assert num_samples == num_components == 1
            z_2nd = torch.reshape(z_2nd, (-1, self.latent.z_dim))
            zx_2nd = torch.cat([z_2nd, x.repeat(num_samples * num_components, 1)], dim=1)
        ...
        if self.hyperparams['incl_robot_node']:
            ...
            # HEDI: for each bach element, input_ contains the current position and its associated node
        ...
        for j in range(ph):
            ...
            if z_2nd is not None:
                switch_flag = ((j - switch_t) >= 0).int()  # 1 if switch
                zx = zx_2nd * switch_flag + zx_1st * (1-switch_flag)
        ...
        if mode in [ModeKeys.PREDICT, ModeKeys.GAN]:
            ...
            sampled_future = self.dynamic.integrate_samples(a_sample, x) #HEDI: if a_dist is the position distribution, why do we integrate ?
    ...
    def encoder(self, mode, x, y_e, num_samples=None):
        ...
        """
        if mode in [ModeKeys.TRAIN, ModeKeys.GAN]:
    ...
    def decoder(self, mode, x, x_nr_t, y, y_r, n_s_t0, z, labels, prediction_horizon, num_samples):
        ...
        if mode in [ModeKeys.PREDICT, ModeKeys.GAN]:
            return log_p_y_xz, sampled_future
        else:
            return log_p_y_xz
    ...
    def train_loss(self,
        ...
        z, kl = self.encoder(mode, x, y_e) # HEDI: kl between q(z|x,y) and p(z|x)
    ...
    def predict(self,
        ...
        mode_prob = self.latent.p_dist.probs
    ...
    def gan_dc_loss(self,
                    ...
                    map,
                    prediction_horizon,
                    grid_std,  # std of the gaussian dist in traj grid
                    grid_max) -> torch.Tensor:
        """
        Calculates the GAN discriminator loss for a batch.
        ...
        initial_node_pos = inputs[:, -1, 0:2].unsqueeze(1)
        initial_node_head = inputs[:, -1, self.hyperparams['map_encoder']['VEHICLE']['heading_state_index']]

        rot_mat = torch.stack([torch.stack([torch.cos(initial_node_head), torch.sin(initial_node_head)]),
                               torch.stack([-torch.sin(initial_node_head), torch.cos(initial_node_head)])]).permute(2, 0, 1)
        patch_size = self.hyperparams['map_encoder']['VEHICLE']['patch_size']

        # discriminator loss
        self.latent.p_dist = self.q_z_xy(mode, x, y_e)  # the sample function of q_dist is fake, use p_dist
        z, num_samples, num_components = self.latent.sample_p(num_samples=1,
                                                              mode=mode,
                                                              most_likely_z=False,
                                                              full_dist=False,
                                                              all_z_sep=False)
        ...
        sample_patch = traj_fit_patch(our_sampled_future.squeeze(0),
                                      initial_node_pos, rot_mat, patch_size)[..., None, None]
        sample_grid = self.get_traj_grid(sample_patch, grid_std, grid_max)
        if map is None:
            bsize,_,h,w = sample_grid.shape
            map_z = torch.zeros(bsize,3,h,w,device=self.device)
            sample_grid_map = torch.cat([map_z, sample_grid], dim=1)
        else:
            sample_grid_map = torch.cat([map * 2 - 1, sample_grid], dim=1)

        labels_patch = traj_fit_patch(labels, initial_node_pos, rot_mat, patch_size)[..., None, None]
        labels_grid = self.get_traj_grid(labels_patch, grid_std, grid_max)
        if map is None:
            labels_grid_map = torch.cat([map_z, labels_grid], dim=1)
        else:
            labels_grid_map = torch.cat([map * 2 - 1, labels_grid], dim=1)

        D_fake, DC_fake = self.node_modules['VEHICLE/discriminator'](sample_grid_map)
        D_real, _ = self.node_modules['VEHICLE/discriminator'](labels_grid_map)

        _, z_label = z[0].max(dim=-1)
        Class_loss = torch.nn.functional.cross_entropy(DC_fake, z_label)

        D_fake_loss = ((D_fake - 0) ** 2).mean() / 2
        D_real_loss = ((D_real - 1) ** 2).mean() / 2

        D_diff = (D_real - D_fake).mean()

        D_loss = D_real_loss + D_fake_loss

        if self.log_writer is not None:
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'D_diff'),
                                       D_diff.mean(),
                                       self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'D_loss'),
                                       D_loss,
                                       self.curr_iter)
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'DC_loss'),
                                       Class_loss, self.curr_iter)

        return D_loss, Class_loss, D_real, D_fake

    def gan_gc_loss(self,
                    inputs,
                    inputs_st,
                    first_history_indices,
                    labels,
                    ...
                    grid_std,
                    grid_max,
                    creative=None) -> torch.Tensor:
        """
        Calculates the GAN generator loss for a batch.
        ...
        if creative is None:
            ...
            z_2nd = OneHotCategorical(logits=self.latent.p_dist.logits + z.permute(1, 0, 2) * (-1e4)).sample([1])[
                0].squeeze(1)
            mid_t = prediction_horizon // 2
            switch_t_shape = [z_2nd.shape[0], 1]
            if 'mid' in creative:  # switch z at the middle time steps
                switch_t = torch.ones(switch_t_shape, device=self.device) * mid_t
            elif 'rand' in creative:  # switch z at a random steps
                switch_t = torch.randint(mid_t - 1, mid_t + 2, switch_t_shape, device=self.device)
        ...
        G_score, Class_score = self.node_modules['VEHICLE/discriminator'](sample_grid_map)

        G_loss = ((G_score - 1) ** 2).mean() / 2

        if creative is None:
            _, z_label = z[0].max(dim=-1)
            Class_loss = torch.nn.functional.cross_entropy(Class_score, z_label)
        elif 'uni' in creative: # kl divergence w.r.t uniform z
            Class_score = F.log_softmax(Class_score, dim=-1)
            Class_loss = -(Class_score.sum(-1) / Class_score.shape[-1]).mean()  # KL divergence
        elif '2hot' in creative:  # kl divergence w.r.t 2 hot z
            Class_score = F.log_softmax(Class_score, dim=-1)
            Class_loss = -((Class_score * (z[0] + z_2nd)).sum(-1) / 2).mean()
        else:
            raise KeyError

        if self.log_writer is not None:
            self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'G_loss'),
                                       G_loss, self.curr_iter)
            if creative is None:
                self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'GC_loss'),
                                           Class_loss, self.curr_iter)
            else:
                self.log_writer.add_scalar('%s/%s' % (str(self.node_type), 'Creative_loss'),
                                           Class_loss, self.curr_iter)

        return G_loss, Class_loss

    def produce_traj(self,
                     inputs,
                     inputs_st,
                     first_history_indices,
                     labels,
        ...
        z, kl = self.encoder(mode, x, y_e)  # z is a sample intead of a distribution
        log_p_y_xz, our_sampled_future = self.decoder(mode, x, x_nr_t, y, y_r, n_s_t0, z,
                                                      labels,
                                                      prediction_horizon,
                                                      1)
    ...
    def visualization_gan(self,
                          ...
                          map,
                          prediction_horizon,
                          grid_std=10,  # std of the gaussian dist in traj grid
                          grid_k=200,
                          lambda_gp=10):
        """
        :param inputs: Input tensor including the state for each agent over time [bs, t, state].
        ...
        return our_sampled_future, labels, map, partial(traj_fit_patch, init_pos=initial_node_pos, rot_mat=rot_mat, patch_size=patch_size)


    def get_traj_grid(self, traj_patch, std, max_v):
        patch_size = self.hyperparams['map_encoder']['VEHICLE']['patch_size']
        width = patch_size[0] + patch_size[2]
        height = patch_size[1] + patch_size[3]

        sampled_future_grid = torch.stack(torch.meshgrid(
            torch.linspace(0, width - 1, width, device=self.device),
            torch.linspace(0, height - 1, height, device=self.device))
        #).repeat(6, 1, 1, 1)
        ).repeat(self.hyperparams["prediction_horizon"], 1, 1, 1)

        #import ipdb
        #ipdb.set_trace()

        traj_grid = torch.exp(torch.sum(torch.distributions.normal.Normal(
            traj_patch, std * torch.ones_like(traj_patch)).log_prob(sampled_future_grid), dim=-3))

        traj_grid_scale = traj_grid / traj_grid.max() * max_v

        return traj_grid_scale
