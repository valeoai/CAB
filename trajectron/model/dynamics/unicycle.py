...
class Unicycle(Dynamic):
    ...
    def create_graph(self, xz_size):
        ...
        self.p0_model = self.model_registrar.get_model(f"{self.node_type}/{self.branch}/unicycle_initializer", model_if_absent)
    ...
    def integrate_distribution(self, control_dist_dphi_a, x):
        ...
        phi_0 = phi_0 + torch.tanh(self.p0_model(torch.cat((x, phi_0), dim=-1))) # HEDI: I don't understand what the p0_model has to do here
        ...
        pos_dist_sigma_matrix_t = torch.zeros(sample_batch_dim + [control_dist_dphi_a.components, 4, 4],
                                              device=self.device) #HEDI: is it 4 because of the size of internal state of the unicycle model ? [x, y, phi, v]

        u = torch.stack([control_dist_dphi_a.mus[..., 0], control_dist_dphi_a.mus[..., 1]], dim=0) #HEDI: controls at each time, given by the neural network
        x = torch.stack([p_0[..., 0], p_0[..., 1], phi_0, torch.norm(v_0, dim=-1)], dim=0) #HEDI: states that we want to infer from dynamics (but we know the first one)

        pos_dist_sigma_matrix_list = []
        mus_list = []
        for t in range(ph):
            # HEDI: update the uncertainty matrix (=covar matrix) of the states (x,y,phi,v)
            F_t = self.compute_jacobian(sample_batch_dim, control_dist_dphi_a.components, x, u[:, :, :, t]) #HEDI: d(state at t+1) / dmu[control] (in 4 x 4)
            G_t = self.compute_control_jacobian(sample_batch_dim, control_dist_dphi_a.components, x, u[:, :, :, t]) #HEDI: d(state at t+1) / dmu[state at t] (in 4x2)
            ...
            pos_dist_sigma_matrix_t = (F_t.matmul(pos_dist_sigma_matrix_t.matmul(F_t.transpose(-2, -1)))
                                       + G_t.matmul(dist_sigma_matrix_t.matmul(G_t.transpose(-2, -1)))) #HEDI: update of the covariance matrix of the states (Sigma_{x, y, phi, v})
            pos_dist_sigma_matrix_list.append(pos_dist_sigma_matrix_t[..., :2, :2]) #HEDI: we only need to keep covariances for the positions

            x = self.dynamic(x, u[:, :, :, t]) # HEDI: update of the states
            mus_list.append(torch.stack((x[0], x[1]), dim=-1)) #HEDI: we only need to keep mus for the positions
