...
def compute_batch_statistics(prediction_output_dict,
    ...
    for node_type in node_type_enum:
        ...
        if nodes_and_ts:
            batch_error_dict[node_type].update({"node":list(),"ts":list()})
    ...
    for t in prediction_dict.keys():
        for node in prediction_dict[t].keys():
            ...
            if displ_avg:
                ade_errors = np.mean(ade_errors, keepdims=True)
                fde_errors = np.mean(fde_errors, keepdims=True)
            ...
            if nodes_and_ts:
                batch_error_dict[node.type]['node'].extend([node])
                batch_error_dict[node.type]['ts'].extend([t])
